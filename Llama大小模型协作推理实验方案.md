...
- 方案 B 中反思重启的有效性
- Group D / CNTP 原论文方法作为独立对照时的准确率与延迟
...
| `configs/group_ac/d_cautious_first100.yaml` | Group D / CNTP first100 对照配置 |
| `scripts/run_group_d_cautious_first100.sh` | Group D / CNTP first100 服务器运行脚本 |
...
| RULER NIAH small | 20 | 长上下文 needle-in-a-haystack 检索，小规模加入 finalist 压力测试 |
...
- RULER NIAH small 参考 NVIDIA/RULER 的 synthetic NIAH schema，使用 `input` / `outputs` / `length` JSONL 字段；本项目先用 `scripts/prepare_ruler_small.py` 生成确定性的 20 条约 2K 词上下文样本，作为 finalist 的小规模长上下文压力测试。
...
### 4.5 Group D：CNTP / Cautious Next Token Prediction 论文方法对照

Group D 作为 **CNTP 原论文方法的对照组**，不作为当前 Group A/C 的优化目标。它的作用是回答：

```text
在相同数据、相同大模型、相同评分函数下，CNTP 原论文的 cautious multi-trial decoding
相对于 Baseline、Group B、Group A、Group C 的准确率和延迟表现如何？
```

实现口径：

- 使用项目 `ASPS/` 中保留的 CNTP patched `transformers` 生成逻辑。
- 生成脚本为 `ASPS/experiments/SelfEval-Guided-Decoding/src/generate_code_groupd_cautious_llama.py`。
- 训练入口为根目录 `train.py` 的 `group_d`。
- 默认配置为 `configs/group_ac/d_cautious_first100.yaml`。
- 服务器运行脚本为 `scripts/run_group_d_cautious_first100.sh`。
- 结果汇总通过 `scripts/collect_results.py` 读取 `groupd_metrics`。

默认 first100 参数：

| 参数 | 值 |
|---|---:|
| cntp mode | `perplexity` |
...
注意：Group D 必须运行在 ASPS custom transformers 环境中。服务器 `git pull` 后，如果 editable install 仍指向旧路径，需要重新安装：
...
Group D 脚本启动时会检查当前 `transformers` 是否包含 CNTP 分发和 `_sample_reflect_perplexity*` 方法；如果没有安装 patched 包，会直接报错，避免误跑成普通 decoding。
...
| 指标 | 含义 |
|---|---|
| cntp mode | CNTP 解码模式，默认 `perplexity` |
| entropy_threshold_low | CNTP 低 entropy 阈值 |
| entropy_threshold_high | CNTP 高 entropy 阈值 |
| max_trials | 高不确定位置最多探索次数 |
| avg sample wall time | 单题平均 wall-clock time |
| p90 sample wall time | 单题 P90 wall-clock time |
| job duration | 每个数据集完整运行耗时 |
| accuracy | 和 Baseline、Group B、Group A/C 在同题集合上的准确率对比 |
...
18. Group D 只能作为 CNTP 论文方法对照，不参与 Group A/C 的参数最优选择。
19. Group D 必须使用和其他文本组相同的 `load_dataset_examples`、答案抽取与 `score_prediction`，保证对比只来自解码策略差异。
20. Group D 必须显式启用 CNTP flag，并在运行时检查 patched `transformers`，避免误用普通 HuggingFace decoding。
...
| Group D CNTP mode | perplexity |
| Group D entropy low / high | 0.01 / 1.5 |
| Group D max trials | 10 |
...
8. Group D / CNTP 原论文方法在同题集合上是否优于 Baseline、Group B、Group A/C？
...
- Group D 预计比 Baseline 更慢，因为它会在高不确定位置探索多路径；它的价值主要是作为 CNTP 论文方法的同环境对照。
...
| 实验组 | Baseline + Group A + Group C + Group B，Group D 可作为单独对照补跑 |
| 预估时间 | 1-3 小时 |
...
| 实验组 | Baseline + Group A + Group C + Group B，Group D / CNTP 作为单独 first100 对照 |
| seed | 1 个 |
| 预估纯运行时间 | 6-16 小时 |
